...
type Operator = '+' | '-';
type Unit = '%';
type PercentageParser<A extends string> = A extends `${Operator}${infer L}`
  ? A extends `${infer O}${infer N}${Unit}` // 有符号的情况
    ? [O, N, Unit] // 有符号并且有 % 
    : A extends  `${infer O}${infer N}` // 有符号没有 %
      ?  [O, N, '']
      :never
  : A extends `${infer P}${Unit}` // 无符号的情况
    ? ['', P, Unit] // 无符号有 %
    : ['', A, '']
