...
type DropSingle<S, R extends string> = S extends `${infer F}${R}${infer L}` ? DropSingle<`${F}${L}`, R> : S;
type DropString<S extends string, R extends string> = R extends `${infer First}${infer Rest}` ? DropString<DropSingle<S, First>, Rest> : S
