...
type LengthOfString<S extends string, R extends 0[] = []> =
  S extends `${infer A}${infer B}${infer C}${infer D}${infer E}${infer F}${infer G}${infer H}${infer Rest}`
    ? LengthOfString<Rest, [0, 0, 0, 0, 0, 0, 0, 0, ...R]>
    : S extends `${infer _}${infer Rest2}`
      ? LengthOfString<Rest2, [0, ...R]>
      : R['length']
